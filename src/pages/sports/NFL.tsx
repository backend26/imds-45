...
export default function NFL() {
  ...
  useEffect(() => {
    const fetchNFLPosts = async () => {
      try {
        setLoading(true);
        
        // First, get the category ID for NFL
        ...
        if (!categoryData) {
          console.warn('Categoria NFL non trovata');
          setPosts([]);
          return;
        }
        ...
        console.error('Error fetching NFL posts:', error);
        setPosts([]);
      } finally {
        setLoading(false);
      }
  ...
  const mapPostToCard = (post: any) => {
    ...
    return {
      ...
      category: post.categories?.name || 'NFL',
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl">
            Le ultime notizie dalla National Football League, analisi delle partite, 
            aggiornamenti sui playoff e il Super Bowl.
          </p>
        </div>
            ...
            {posts.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Non ci sono ancora articoli di NFL disponibili. Torna presto per gli aggiornamenti!
                </p>
              </div>
            )}
          </>
