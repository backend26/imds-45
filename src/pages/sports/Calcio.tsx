...
export default function Calcio() {
  ...
  useEffect(() => {
    const fetchCalcioPosts = async () => {
      try {
        setLoading(true);
        
        // First, get the category ID for Calcio
        ...
        if (!categoryData) {
          console.warn('Categoria Calcio non trovata');
          setPosts([]);
          return;
        }
        ...
        console.error('Error fetching calcio posts:', error);
        setPosts([]);
      } finally {
        setLoading(false);
      }
  ...
  const mapPostToCard = (post: any) => {
    ...
    return {
      ...
      category: post.categories?.name || 'Calcio',
