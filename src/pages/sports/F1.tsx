...
export default function F1() {
  ...
  useEffect(() => {
    const fetchF1Posts = async () => {
      try {
        setLoading(true);
        
        // First, get the category ID for F1
        ...
        if (!categoryData) {
          console.warn('Categoria F1 non trovata');
          setPosts([]);
          return;
        }
        ...
        console.error('Error fetching F1 posts:', error);
        setPosts([]);
      } finally {
        setLoading(false);
      }
  ...
  const mapPostToCard = (post: any) => {
    ...
    return {
      ...
      category: post.categories?.name || 'F1',
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <Badge className="bg-gradient-to-r from-red-500 to-red-700 text-white text-lg px-4 py-2">
              🏎️ Formula 1
            </Badge>
          </div>
          ...
          <p className="text-xl text-muted-foreground max-w-3xl">
            Le ultime notizie dal mondo della Formula 1, analisi delle gare, 
            aggiornamenti sui piloti e le scuderie.
          </p>
        </div>
