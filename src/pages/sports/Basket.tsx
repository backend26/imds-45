...
export default function Basket() {
  ...
  useEffect(() => {
    const fetchBasketPosts = async () => {
      try {
        setLoading(true);
        
        // First, get the category ID for Basket
        ...
        if (!categoryData) {
          console.warn('Categoria Basket non trovata');
          setPosts([]);
          return;
        }
        ...
        console.error('Error fetching basket posts:', error);
        setPosts([]);
      } finally {
        setLoading(false);
      }
  ...
  const mapPostToCard = (post: any) => {
    ...
    return {
      ...
      category: post.categories?.name || 'Basket',
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <Badge className="bg-gradient-to-r from-orange-500 to-amber-600 text-white text-lg px-4 py-2">
              🏀 Basket
            </Badge>
          </div>
          ...
          <p className="text-xl text-muted-foreground max-w-3xl">
            Le ultime notizie dalla NBA, dalla Serie A italiana e dai campionati internazionali. 
            Analisi, statistiche e aggiornamenti sui tuoi giocatori preferiti.
          </p>
        </div>
            ...
            {posts.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Non ci sono ancora articoli di basket disponibili. Torna presto per gli aggiornamenti!
                </p>
              </div>
            )}
          </>
